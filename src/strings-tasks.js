...
function orderAlphabetically(str) {
  ...
  const sortArray = arrayOfElements.sort();
  for (let i = 0; i < sortArray.length; i += 1) {
    inOreder += sortArray.at(i);
  }
  return inOreder;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!punctuationMarks.includes(str.at(i))) {
      strOfElements += str.at(i);
    }
  }
  const strOfLowerElements = strOfElements.toLowerCase();
  for (let k = 0; k < strOfLowerElements.length; k += 1) {
    if (strOfLowerElements.at(k) !== strOfLowerElements.at(-k - 1)) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  const wordsArroy = sentence.split(' ');
  let lettersCount = 0;
  let word = '';
  for (let i = 0; i < wordsArroy.length; i += 1) {
    if (wordsArroy.at(i).length > lettersCount) {
      lettersCount = wordsArroy.at(i).length;
      word = wordsArroy.at(i);
    }
  }
  return word;
}
...
function encodeToRot13(str) {
  ...
  const punctuationMarks = [' ', ',', '.', '!', '?'];
  for (let i = 0; i < str.length; i += 1) {
    let uppercaseСode = str.codePointAt(i) - 13;
    if (uppercaseСode < 65) {
      uppercaseСode = 91 - (65 - uppercaseСode);
    }
    let lowercaseСode = str.codePointAt(i) + 13;
    if (lowercaseСode > 122) {
      lowercaseСode = 96 + (lowercaseСode - 122);
    }
    if (str.codePointAt(i) >= 65 && str.codePointAt(i) <= 90) {
      result += String.fromCodePoint(uppercaseСode);
    } else if (str.codePointAt(i) >= 97 && str.codePointAt(i) <= 122) {
      result += String.fromCodePoint(lowercaseСode);
    }
    for (let j = 0; j < punctuationMarks.length; j += 1) {
      if (str.at(i) === punctuationMarks.at(j)) {
        result += punctuationMarks.at(j);
      }
    }
  }
  return result;
